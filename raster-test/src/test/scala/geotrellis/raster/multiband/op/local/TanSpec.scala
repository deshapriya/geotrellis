...
class TanSpec extends FunSpec
  ...
  describe("Tan MultiBandTile") {
    it("finds the Tan of a double multiband raster") {
      ...
      val t6 = DoubleArrayTile(arr0.map(math.tan(_)), 3, 2)
      val t7 = DoubleArrayTile(arr1.map(math.tan(_)), 3, 2)
      val t8 = DoubleArrayTile(arr2.map(math.tan(_)), 3, 2)
      val t9 = DoubleArrayTile(arr3.map(math.tan(_)), 3, 2)
      val t10 = DoubleArrayTile(arr4.map(math.tan(_)), 3, 2)
      val t11 = DoubleArrayTile(arr5.map(math.tan(_)), 3, 2)
      ...
      for (band <- 0 until r.bands) {
        for (col <- 0 until r.cols) {
          for (row <- 0 until r.rows) {
            val tan = r.getBand(band).getDouble(col, row)
            val epsilon = math.ulp(tan)
            val ep = ex.getBand(band).getDouble(col, row)
            if (m.getBand(band).getDouble(col, row).isNaN())
              tan.isNaN() should be(true)
            else if (m.getBand(band).getDouble(col, row).isInfinite())
              tan.isNaN() should be(true)
            else
              tan should be(ep +- epsilon)
          }
        }
      }
    }

    it("finds the tan of an int multiband raster") {
      ...
      val t2 = DoubleArrayTile(a0.map(math.tan(_)), 4, 2)
      val t3 = DoubleArrayTile(a1.map(math.tan(_)), 4, 2)
