...
class SinhSpec extends FunSpec
  ...
  describe("Sinh MultiBandTile") {
    it("finds the hyperbolic sine of a int multiband raster") {
      ...
      val t2 = DoubleArrayTile(a0.map(math.sinh(_)), 4, 2)
      val t3 = DoubleArrayTile(a1.map(math.sinh(_)), 4, 2)
      ...
      for (band <- 0 until r.bands) {
        for (col <- 0 until r.cols) {
          for (row <- 0 until r.rows) {
            val sinh = r.getBand(band).getDouble(col, row)
            val epsilon = math.ulp(sinh)
            val ep = ex.getBand(band).getDouble(col, row)
            if (isNoData(m.getBand(band).get(col, row)))
              sinh.isNaN() should be(true)
            else
              sinh should be(ep +- epsilon)
          }
        }
      }
    }

    it("finds the hyperbolic sine of a double multiband raster") {
      ...
      val t6 = DoubleArrayTile(arr0.map(math.sinh(_)), 3, 2)
      val t7 = DoubleArrayTile(arr1.map(math.sinh(_)), 3, 2)
      val t8 = DoubleArrayTile(arr2.map(math.sinh(_)), 3, 2)
      val t9 = DoubleArrayTile(arr3.map(math.sinh(_)), 3, 2)
      val t10 = DoubleArrayTile(arr4.map(math.sinh(_)), 3, 2)
      val t11 = DoubleArrayTile(arr5.map(math.sinh(_)), 3, 2)
