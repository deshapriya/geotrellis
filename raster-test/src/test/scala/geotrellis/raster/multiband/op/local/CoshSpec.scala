...
class CoshSpec extends FunSpec
  ...
  describe("Cosh MultiBandTIle") {
    it("finds the hyperbolic cosine of a double multiband raster") {
      ...
      val t6 = DoubleArrayTile(arr0.map(math.cosh(_)), 3, 2)
      val t7 = DoubleArrayTile(arr1.map(math.cosh(_)), 3, 2)
      val t8 = DoubleArrayTile(arr2.map(math.cosh(_)), 3, 2)
      val t9 = DoubleArrayTile(arr3.map(math.cosh(_)), 3, 2)
      val t10 = DoubleArrayTile(arr4.map(math.cosh(_)), 3, 2)
      val t11 = DoubleArrayTile(arr5.map(math.cosh(_)), 3, 2)
      ...
      for (band <- 0 until r.bands) {
        for (col <- 0 until r.cols) {
          for (row <- 0 until r.rows) {
            val cosh = r.getBand(band).getDouble(col, row)
            val epsilon = math.ulp(cosh)
            val ep = ex.getBand(band).getDouble(col, row)
            if (m.getBand(band).getDouble(col, row).isNaN())
              cosh.isNaN() should be(true)
            else if (cosh.isInfinite())
              cosh should be(ep)
            else
              cosh should be(ep +- epsilon)
          }
        }
      }
    }

    it("finds the hyperbolic cosine of an int multiband raster") {
      ...
      val t2 = DoubleArrayTile(a0.map(math.cosh(_)), 4, 2)
      val t3 = DoubleArrayTile(a1.map(math.cosh(_)), 4, 2)
